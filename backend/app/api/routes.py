...
@router.post("/datascience/analyses/{analysis_id}/schedule-report", status_code=201)
async def schedule_analysis_report(
    analysis_id: str, payload: ScheduleReportRequest
) -> dict:
    """Generate a scheduled workflow: rerun the analysis, optionally notify."""
    analysis = _get_analysis_or_404(analysis_id)
    trigger_config: dict = {"trigger_kind": "schedule"}
    if payload.cron:
        problem = cron_error(payload.cron.strip())
        ...
        trigger_config["interval_seconds"] = max(300, payload.interval_seconds)
    ...
    edges: list[dict] = []
    last = "report"
    if payload.slack_credential and payload.slack_channel:
        nodes.append(
            {
                "id": "notify",
                "type": "connector",
                ...
                "config": {
                    "connector": "slack",
                    "action": "send_message",
                    "credential": payload.slack_credential,
                    "params": {
        ...
        edges.append({"source": "report", "target": "notify"})
        last = "notify"
    nodes.append(
        {
            "id": "end",
    ...
    doc = WorkflowDoc(
        name=f"Report: {analysis['name']}",
        description=(
            f"Generated by Data Science: reruns analysis '{analysis['name']}' "
    ...
    saved = get_store().create(doc)
    await _sync_schedule(saved)
    return {"workflow_id": saved.id, "workflow_name": saved.name}


# ---------------------------------------------------------------- ML models


@router.get("/datascience/models")
...
@router.post("/datascience/models/train", status_code=201)
def train_model_endpoint(payload: ModelTrainRequest) -> dict:
...
@router.post("/datascience/models/{model_id}/predict")
def predict_endpoint(model_id: str, payload: ModelPredictRequest) -> dict:
    from app.ml import predict_with_model

    try:
        return predict_with_model(
            model_id, payload.source, payload.dataset, payload.mode
        )
    except LookupError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
    except ValueError as exc:
...
@router.delete("/datascience/models/{model_id}", status_code=204)
